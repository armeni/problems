...
class EmbeddedRosenbrockMethod(OneStepMethod):
    ...
    def embedded_step(self, func: ODE, t, y, dt):
        ...
        n = np.size(b)
        k = np.zeros((n, len(y)))
        I = np.linalg.inv(np.eye(len(y)) - g * dt * func.jacobian(t, y))
        for i in range(n):
            k[i] = I.dot(dt * func(t + dt * c[i], y + np.dot(A[i], k)) + dt * func.jacobian(t, y).dot(np.dot(G[i], k)))
        y1, dy = y + b.dot(k), e.dot(k)
        return y1, dy
